...
class ResizableWindow:
    def __init__(self, parent):
        self.parent = parent
        self.f1_style = ttk.Style() # initiate styles from ttk module https://docs.python.org/3/library/tkinter.ttk.html
        ...
        self.parent.eval('tk::PlaceWindow %s center' % parent.winfo_pathname(parent.winfo_id())) # center program on screen
        self.parent.geometry("400x200")
        self.parent.resizable(False, False) # do not allow window to resize
        self.parent.title("WPCT") # title=website parser comparision tool
 
        self.f1.grid(column=0, row=0, sticky=(N, S, E, W))  # added sticky
        #self.f2 = ttk.Frame(self.f1, borderwidth=5, relief="sunken", width=200, height=100) # this is the border
        ...
        self.f1.grid(column=0, row=0, sticky=(N, S, E, W), padx=5)  # added sticky
        #self.f2.grid(column=0, row=0, columnspan=3, rowspan=2, sticky=(N, S, E, W))  # added sticky
        self.namelbl.grid(column=1, row=0, columnspan=2, sticky=(N, W), padx=5)  # added sticky, padx
        self.name.grid(column=1, row=1, columnspan=2, sticky=(N, E, W), pady=5, padx=5)  # added sticky, pady, padx
